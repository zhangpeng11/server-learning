...
define(function(require, exports, module) {
    ...
    //全局变量
    var wph = {
        b_url: $.zheui.protocol + "//m.zhe800.com/m/brand/list?",
    ...
    */
        ...
        init: function() {
                    ...
                    if (_this.allmethod.view_didappear) {
                        ...
                        $.calljs.view_didappear_callback = function() {
                            ...
                            if (currli.attr("source_type") == "1") {
                                if (status == 1) {
                                    //设置成功，已经设置，返回按钮变灰
                                    currli.find(".buttons").attr("data-alarm", 'cancel_alarm');
                                    currli.find(".buttons span").addClass("alarm");
                                }
                                if (status == 2) {
                                    //取消成功的按钮，恢复正常的状态
                                    currli.find(".buttons").attr("data-alarm", 'set_alarm');
                                    currli.find(".buttons span").removeClass("alarm");
                                }
                            } else {
            ...
            if (page_from == "zheclient") {
                ...
                if(useragent.indexOf('OPPO R7') > -1){
                    $('.subnavlist .selected i').css('margin-top','-8px');
                }
                else{
                    $('.subnavlist .selected i').css('margin-top','-4px');
                }
      ...
      */
        ...
        navCur: function(ind) {
          ...
          $(".slide_img").show();
           _this.$slideImg.height()==0?$(".list_all .wrap_content").css('margin-top','42.5px'):$(".list_all .wrap_content").css('margin-top','0px');
            ...
            //在客户端适配OPPO R7机型
            var useragent=navigator.userAgent;
            if(useragent.indexOf('OPPO R7') > -1 && page_from == "zheclient"){
                $('.subnavlist .selected i').css('margin-top','-8px');
            }
            else{
                $('.subnavlist .selected i').css('margin-top','-4px');
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < len; i++) {
                        ...
                        if (navData[i].now_count != 0) {
                            ...
                            itemStr += '<div class="conItem list_' + navData[i].url_name + '"><div class="wrap_content"><ul class="brand_list"></ul></div><div class="loading_more"><span class="loading"><i class="icon_load"></i>加载中......</span></div><div class="bottom-pic"></div></div>';
        ...
        // 限时抢购
        getTimeSell: function() {
            var _this = this;
            browserWidth = $('body').width();
            var url = '';
            var userType = 0;
            //将协议新老用户信息同步接口,接口1新用户，其他为老用户
            if(_this.config.userInfo.userType != 1){
                userType = 1;
            }
            else{
                userType = 0;
            ...
            $.ajax({
                type: "get",
                url: '//www.zhe800.com/api/get_xianshiqiang_jxyg',
                data: { 
                    new_user: userType
                },
                dataType: "jsonp",
                jsonp: "callback",
                success: function (data) {
                    var ele = $('.time_sell_list').find('ul');
                    if ($.isPlainObject(data) && data.deals.length >= 3) {
                        $('.time_sell').show();
                        var s = '', list = document.querySelector('.time_sell_list ul');
                        var curTime = _this.formatDate(new Date(data.now));
                        data.deals.forEach(function (item) {
                            var price = '', id='',source_type='';
                            var id = item.id,
                            list_price = item.list_price,
                            begin_time = _this.formatDate(new Date(item.begin_time)),
                            wap_url = 'http://g.hd.zhe800.com/xianshiqiang/index?deal_id='+id+'&new_user='+userType,
                            beginDay = begin_time.substring(0, 10);
                            if(item.xianshi.activity_type == 100 || item.xianshi.activity_type == 99){
                                price = item.price;
                            }else{
                                price = item.xianshi.prices.wireless;
                            }
                            //区分淘宝天猫和特卖商城
                            if(item.xianshi){
                                s += '<li data-id='+id+' wap_url='+wap_url+'>'
                                +'<div class="sale_img"><img src="//i0.tuanimg.com/ms/zhe800m/dist/img/img_placehd2.png" data-url="'+item.origin_square_image_url+'" width="100" height="100" style="width:100px;height:100px"/></div>'
                                +'<div class="sale_info"><i class="ico_price"><span class="yuan">￥</span>'+ price + '</i><i class="list_price"><span class="list_yuan">￥</span>' +list_price+'</i>';
                                if (curTime.substring(0, 10) == beginDay){
                                    s += '<div class="presell_time"><span>今天'+begin_time.substring(11, 16)+'</span></div>';
                                } 
                                else {
                                    var countTime = (new Date(begin_time.substring(0, 10).replace(/-/g, "/")).getTime() - new Date(curTime.substring(0, 10).replace(/-/g, "/")).getTime());
                                    if (countTime <= 3600 * 24 * 1000) {
                                        s += '<div class="presell_time"><span>明天'+begin_time.substring(11, 16)+'</span></div>';
                                    } else if (countTime > 3600 * 24 * 1000 && countTime <= 3600 * 24 * 2 * 1000) {
                                        s += '<div class="presell_time"><span>后天'+begin_time.substring(11, 16)+'</span></div>';
                                    } else {
                                        s += '<div class="presell_time"><span>'+beginDay+'</span></div>';
                                    }
                                }
                                s += '</div></li>';
                            }
                        ...
                        //绑定点击事件
                        _this.bindTouchEvent($('.time_sell').find("li"), function(obj) {
                            var $this = obj,
        ...
        domBanner: function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    _this.$slideImg.height() == 0 ? $(".list_all .wrap_content").css('margin-top', '0px') : $(".list_all .wrap_content").css('margin-top', '0px');
        ...
        winScroll: function() {
            ...
            $(window).bind("scroll", function() {
                ...
                if (scrollTop + _this.winH + 500 >= conH && _this.config.listData[curName].nextPage && _this.config.listData[curName].tempPoint != conH) {
            ...
            $(window).bind('scroll', function(){
                //对滑动banner显示状态进行区分
                if(slide_img.height() == 0){
                    scrollHeight = $('.time_sell').height();
                }
                else{
                    scrollHeight = slide_img.height() + slide_img.offset().top + $('.time_sell').height();
                }
        ...
        tabFn: function() {
            var _this = this;
            this.swipeObj = new Swipe($('.swipe').get(0), {
                startSlide: 0,
                continuous: false,
                disableScroll: false,
                ...
                transitionEnd: function(index, element) {
                    ...
                    _this.$conItem.find(".wrap_content .brand_list").not(".list_" + urlName + " .wrap_content .brand_list").html("");
            ...
            this.bindTouchEvent(_this.$navlist_down, function(obj, index) {
                ...
                $(".sels_down ul").animate({
                    "top": (-1) * ($('.subnavlist').height() + $('.subnavlist').offset().top + _this.subNavHeight)
      ...
      */
        navupdown: function() {
            ...
            this.bindTouchEvent($(".opcity"), function() {
                ...
                _this.$navWrapdown.animate({
                    top: (-1) * ($('.subnavlist').offset().top + $('.subnavlist').height() + _this.subNavHeight)
                }, 200);
                $(".icon_down").animate({
                    "-webkit-transform": 'rotateZ(-360deg)',
                    "transition": "all 0.2s ease-in-out"
            ...
            this.bindTouchEvent($(".navdown"), function() {
                var nav_height = _this.up_flag ? 0 : (-1) * ($('.subnavlist').offset().top + $('.subnavlist').height() + _this.subNavHeight);
                _this.up_flag = !_this.up_flag;
                _this.$navWrapdown.animate({
                    top: nav_height
                }, 200);
        ...
        //分割小数
        splitPrice: function(priceValue){
            var _this = this;
            var price = _this.priceFormat(priceValue).toString();
            return price.split('.');
        },
        /**
         * 价格转换为元，小数点后的无效0不要显示
        ...
        listInit: function(urlName, pageInd, callback) {
            ...
            var _this = this,
                ...
                url = '',
                data = {};
            url = '/deals/forecast/v1';
            data = {
                page: pageInd,
                per_page: 20,
                url_name: urlName,
                user_type: _this.config.userInfo.userType,
                user_role: _this.config.userInfo.userRole,
                student: _this.config.userInfo.student,
                image_type: 'si3',
            },
            ...
            $.ajax({
                ...
                $(".slide_img").show();
                 ...
                 $(".slide_img").hide();
                    ...
                    _this.config.listData[urlName].count = data.meta.count;
                    // }
                    //调用接口后将标志位重置
                    _this.hasItem = false;
                    ...
                            var id = dataObj[i].id,                              //商品id/品牌专场id/banner id,
                            title = dataObj[i].title,                            //名称, 在不同情况下展示不同文案: 专场形式: 展示专题文案 单坑形式: 展示单坑标题文案,
                            begin_time = dataObj[i].begin_time,                  //开始时间,
                            beginDay = begin_time.substring(0, 10),
                            expire_time = dataObj[i].expire_time,                //失效时间,
                            oos = dataObj[i].oos,                                //是否卖光，0代表未卖光, 1代表已卖光, 目前只有单坑商品使用
                            wap_url = dataObj[i].wap_url,                        //命中的url
                            image_url = dataObj[i].image_url,                    //图片访问url；在不同情况下提供不同url；专场形式：展示专题图片 单坑形式：展示单坑图片,
                            image_url = (_this.isWebp && image_url.indexOf(".jpg") != -1) ? image_url + ".webp" : image_url;
                            icon_list = dataObj[i].icon_list,                    //列表左上角标签,
                            icon_grid = dataObj[i].icon_grid,                    //宫格左上角标签,
                            static_key_id = dataObj[i].static_key_id,            //运营位统计信息，只在大数据推荐时返回，且商品是自定义运营位商品或列表banner,
                            item_attribute_id = dataObj[i].item_attribute_id;    //统计打点信息,

                            if(dataObj[i].deal){
                                var  source_type ='',
                                price = dataObj[i].deal.price,
                                wap_url = 'http://out.zhe800.com/m/deal/'+id,
                                shop_type = dataObj[i].deal.shop_type,
                                alarmClass = '';
                                _this.hasItem = true;
                                var promotion_text = dataObj[i].deal.promotion_text;
                                var coupon_price = dataObj[i].deal.coupon_info.coupon_price;
                                var coupon_text = dataObj[i].deal.coupon_info.coupon_text;
                                //如果存在cpc优惠券，则计算券后价
                                if(coupon_price){
                                ...
                                if(source_type){
                                    ...
                                    htmlStr += '<li class="deal li_'+id+'" data-id="'+id+'" data-type="deal" source_type="'+source_type+'" wap_url="'+wap_url+'" begintime="'+begin_time+'" expiretime="'+expire_time+'" oos="'+oos+'"><div class="li_bwrap"><b class="icon_grid" style="background:url('+icon_grid+') no-repeat 0 0;background-size: 100%;"></b><div class="img"><img src="//i0.tuanimg.com/ms/zhe800m/dist/img/img_placehdv2.jpg" data-url="' + image_url +'"/></div><div class="info"><div class="title"><span>'+title+'</span></div>';
                                    htmlStr += '<div class="price"><span class="decimal">￥</span><span class="integer">'+ _this.splitPrice(price)[0] +'</span>';
                                    if(_this.splitPrice(price)[1]){
                                        htmlStr += '<span class="decimal">.'+ _this.splitPrice(price)[1] +'</span>';
                                    }
                                    //有券后价且无优惠信息
                                    if(price > 0 && !promotion_text && coupon_price){
                                        htmlStr += '<span class="coupon_text">'+coupon_text+'</span>';
                                    }
                                    if(promotion_text){
                                        htmlStr += '<span class="promotion_text">'+ promotion_text +'</span>';
                                    }
                                    if (curTime.substring(0, 10) == beginDay){
                                        htmlStr += '</div><div class="sale_time"><span>今天'+begin_time.substring(11, 16)+'</span></div>';
                                    } 
                                    else {
                                        var countTime = (new Date(begin_time.substring(0, 10).replace(/-/g, "/")).getTime() - new Date(curTime.substring(0, 10).replace(/-/g, "/")).getTime());
                                        if (countTime <= 3600 * 24 * 1000) {
                                            htmlStr += '</div><div class="sale_time"><span>明天'+begin_time.substring(11, 16)+'</span></div>';
                                        } else if (countTime > 3600 * 24 * 1000 && countTime <= 3600 * 24 * 2 * 1000) {
                                            htmlStr += '</div><div class="sale_time"><span>后天'+begin_time.substring(11, 16)+'</span></div>';
                                        } else {
                                            htmlStr += '</div><div class="sale_time"><span>'+beginDay+'</span></div>';
                                        }
                                    }
                                    if (page_from == "zheclient") {
                                    ...
                                    htmlStr += '<div class="buttons" data-alarm='+ (alarmClass == "" ? "set_alarm" : "cancel_alarm")+'><span class='+ (alarmClass == "" ? "" : "alarm")+'></span></div></div></div></li>'; 
                                }                               
                            }
                            if(dataObj[i].brand){
                                var brand_type = dataObj[i].brand.brand_type,          //1: 品牌专场, 2: 店铺团,
                                promotion_info = dataObj[i].brand.promotion_info,      //优惠促销信息,
                                begin_price = dataObj[i].brand.begin_price,            //区间价格的起始价格,
                                expire_price = dataObj[i].brand.expire_price,          //区间价格的结束价格,
                                stock_info = dataObj[i].brand.stock_info;              //库存信息,
                                _this.hasItem = true;
                                //过滤店铺团
                                if(brand_type == "1"){

                                    htmlStr += '<li class="brand li_'+id+'" data-id="'+id+'" data-type="brand" brand_type="'+brand_type+'" wap_url="'+wap_url+'"><div class="li_bwrap"><b class="icon_grid" style="background:url('+icon_grid+') no-repeat 0 0;background-size: 100%;"></b><div class="img"><img src="//i0.tuanimg.com/ms/zhe800m/dist/img/img_placehdv2.jpg" data-url="' + image_url +'"/></div><div class="info"><div class="title"><span>'+title+'</span></div>';
                                    //存在优惠信息优先显示
                                    if(promotion_info){
                                        htmlStr += '<div class="promotion_info"><span>'+promotion_info+'</span></div>';
                                    }
                                    //显示专场区间价
                                    else{
                                        htmlStr += '<div class="price"><span class="begin_price"><span class="decimal">￥</span><span class="integer">'+ _this.splitPrice(begin_price)[0]+'</span>';
                                        if(_this.splitPrice(begin_price)[1]){
                                            htmlStr += '<span class="decimal">.'+ _this.splitPrice(begin_price)[1]+'</span>';
                                        }
                                        htmlStr += '</span>-<span class="expire_price"><span class="begin_price"><span class="decimal">￥</span><span class="integer">'+ _this.splitPrice(expire_price)[0]+'</span>';
                                        if(_this.splitPrice(expire_price)[1]){
                                            htmlStr += '<span class="decimal">.'+ _this.splitPrice(expire_price)[1]+'</span>';
                                        }
                                        htmlStr += '</span></div>';
                                    }

                                    if (curTime.substring(0, 10) == beginDay){
                                        htmlStr += '<div class="sale_time"><span>今天'+begin_time.substring(11, 16)+'</span></div>';
                                    } 
                                    else {
                                        var countTime = (new Date(begin_time.substring(0, 10).replace(/-/g, "/")).getTime() - new Date(curTime.substring(0, 10).replace(/-/g, "/")).getTime());
                                        if (countTime <= 3600 * 24 * 1000) {
                                            htmlStr += '<div class="sale_time"><span>明天'+begin_time.substring(11, 16)+'</span></div>';
                                        } else if (countTime > 3600 * 24 * 1000 && countTime <= 3600 * 24 * 2 * 1000) {
                                            htmlStr += '<div class="sale_time"><span>后天'+begin_time.substring(11, 16)+'</span></div>';
                                        } else {
                                            htmlStr += '<div class="sale_time"><span>'+beginDay+'</span></div>';
                                        }
                                    }
                                    htmlStr += '</div></div></li>';
                                }                               
                            ...
                            if(dataObj[i].banner){
                                _this.hasItem = true;
                                var curNav = $('#navWrap .slide span.selected');
                                if(curNav.attr('url-name') == 'all'){
                                    var banner_type = dataObj[i].banner.banner_type,       //1:图片形式 2:logo聚合形式,
                                    list_image = dataObj[i].banner.list_image,            //列表时使用的图片,
                                    grid_image = dataObj[i].banner.grid_image;            //宫格时使用的图片,
                                    if(banner_type == 1){
                                        htmlStr += '<li class="banner li_'+id+'" data-id="'+id+'" data-type="banner" banner_type="'+banner_type+'" wap_url="'+wap_url+'"><div class="li_bwrap"><img src="//i0.tuanimg.com/ms/zhe800m/dist/img/img_placehdv2.jpg" data-url="' + grid_image +'"/></div></li>';
                                    }
                                    if(banner_type == 2){
                                        var logos = dataObj[i].banner.logos;
                                        htmlStr += '<div class="banner2">';
                                        logos.forEach(function(item){
                                            htmlStr += '<li class="banner li_'+id+'" data-id="'+id+'" data-type="banner" banner_type="'+banner_type+'" wap_url="'+item.url+'"><div class="li_bwrap">';
                                            // htmlStr += '<div class="logo"><div class="img"><img src="//i0.tuanimg.com/ms/zhe800m/dist/img/img_placehdv2.jpg" data-url="' + item.logo +'"/></div><span class="title">'+ item.name +'</span>';
                                            htmlStr += '<div class="logo"><div class="img" style="background-image:url('+ item.logo +')"></div><span class="title">'+ item.name +'</span>';
                                            if(item.tip_type == 1){
                                                var sellDay = item.tip_value.substring(0,10);
                                                if (curTime.substring(0, 10) == sellDay){
                                                    htmlStr += '<div class="sale_time"><span class="tip">今日'+item.tip_value.substring(11, 16)+'</span></div>';
                                                } 
                                                else {
                                                    var countTime = (new Date(item.tip_value.substring(0, 10).replace(/-/g, "/")).getTime() - new Date(curTime.substring(0, 10).replace(/-/g, "/")).getTime());
                                                    if (countTime <= 3600 * 24 * 1000) {
                                                        htmlStr += '<div class="sale_time"><span class="tip">明日'+item.tip_value.substring(11, 16)+'</span></div>';
                                                    } else if (countTime > 3600 * 24 * 1000 && countTime <= 3600 * 24 * 2 * 1000) {
                                                        htmlStr += '<div class="sale_time"><span class="tip">后日'+item.tip_value.substring(11, 16)+'</span></div>';
                                                    } else {
                                                        htmlStr += '<div class="sale_time"><span class="tip">'+sellDay+'</span></div>';
                                                    }
                                                }
                                                htmlStr += '</div>';
                                            }
                                            if(item.tip_type == 2){
                                                htmlStr += '<span class="tip">'+ item.tip_value +'</span></div>';
                                            }
                                            htmlStr += '</div></li>';
                                        });
                                        htmlStr += '</div>';
                        ...
                        if(_this.hasItem){
                            if (pageInd == 1) {
                                currTablist.find(".wrap_content .brand_list").html("");
                            }
                            currTablist.find(".wrap_content .brand_list").append(htmlStr);
                            if (pageInd == 1) {
                            ...
                            _this.$buttons = curr_ul.find(".buttons");
                           ...
                           $(".layer_opcity").height(document.body.offsetHeight+'px');*/
                            ...
                            _this.bindTouchEvent2(_this.$buttons, function(obj) {
            ...
            $.calljs.set_alarmcallback = function(data) {
                ...
                if (data == 1) {
                    ...
                    if (obj.attr("data-alarm") == "cancel_alarm") {
                        ...
                        $(".li_" + id).find(".buttons span").removeClass("alarm");
                        $.common.toast({
                            "text": "取消提醒成功"
                        });
                    } else {
                        ...
                        $(".li_" + id).find(".buttons span").addClass("alarm");
                        $.common.toast({
                            "text": "设置成功，开抢前五分钟提醒你"
                        });
                    }
        ...
        gotoClient: function(obj) {
            ...
            if (page_from == "zheclient") {
                ...
                if(data_type == "brand"){
                    var dealid =  null;
                    if(navigator.userAgent.match(/(Android)\s+([\d.]+)/) || navigator.userAgent.match(/Silk-Accelerated/)){
                        dealid = [""];
                    }
                    else{
                        dealid = [];
            ...
            if (page_from == "m") {
                ...
                if(data_type == 'brand'){
                    ...
                    if(brand_type == 1){
                        var to_url = wph.b_url + "brand_id=" + data_id;
                        to_url = wph.out_url + "url=" + encodeURIComponent(to_url) + "&" + track_data_str;
                        window.location.href = to_url;
                    }
                }
                ...
                //品牌详情页，大促页
                if(data_type == 'banner'){
                    var to_url = url.replace('zhe800://m.zhe800.com/h5/web?url=','');
                    if((/^zhe800:\/\/m\.zhe800\.com\/deal\/brand\/detail.*/).test(url)){
                        var regex = new RegExp("[?&]" + encodeURIComponent('brand_id') + "\\=([^&#]+)");
                        var brand_id = (url.match(regex) || ["", ""])[1];
                        to_url = encodeURIComponent(wph.b_url + "brand_id=" + brand_id);
                    }
                    to_url = wph.out_url + "url=" + to_url + "&" + track_data_str;
                    window.location.href = to_url;
                }
            }
        ...
        gototop: function() {
            ...
            $('#ct').bind({
                ...
                "touchmove": function(e) {
                    ...
                    touch_X = touch_moveEndX - touch_startX;
                    touch_Y = touch_moveEndY - touch_startY;
                    var curName = _this.config.curName;
                    if (Math.abs(touch_X) >= 3 || Math.abs(touch_Y) >= 3) {
                        // isMove = true;
                        if (document.body.scrollTop > 550) {
                            $(".top_con").animate({
                            ...
                            $('.top_con span').remove();
                            $('.top_con').append('<span>'+($('.list_'+curName+' .brand').length + $('.list_'+curName+' .deal').length + $('.list_'+curName+' .banner').length + $('.list_'+curName+' .banner2').length)+'</span><span>'+_this.config.listData[curName].count+'</span>');
                            $('.top_con').css('background-image','none');
                        } else {
                            $(".top_con").animate({
                                display: 'none'
                            }, 10000);
                ...
                "touchend": function(e) {
                    $('.top_con span').css('display','none');
                    $('.top_con').css('background-image','url(//i0.tuanimg.com/ms/zhe800m/dist/img/forecast/top.png);');
                }
            });
        }
    }
    //当用户后台关闭通知选项ios支持手动仅一次设置开卖提醒
    $.calljs.setAlarm = function(id) {
        $(".li_" + id).find(".buttons").attr("data-alarm", "cancel_alarm");
        $(".li_" + id).find(".buttons span").addClass("alarm");
        $.common.toast({
            "text": "保存成功，可在开卖提醒中查看"
